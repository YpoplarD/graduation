...
from reid.utils.data.transforms import ResizeKeepRatio
from reid.models.clipreid_adapter import CLIPReIDAdapter
############################################################

###########################################################

def get_data(name, data_dir, height, width, batch_size, workers, args):
    #######################################3
    ...
    if args.arch.startswith('vit'):
        ...
        ResizeKeepRatio(256, 128),
        T.RandomHorizontalFlip(),
        T.ColorJitter(0.2, 0.1, 0.1, 0),
...
def get_cosine_schedule(optimizer, epochs, warmup_epochs=5):
    def lr_lambda(epoch):
...
def  main(args):
    ...
    dataset, num_classes, train_loader, query_loader, gallery_loader = \
        get_data(args.dataset,  args.data_dir, args.height,
                 args.width, args.batch_size, args.workers,args
                 )
...
#        model.load_state_dict(checkpoint['state_dict'])
    ...
    # Evaluator
    #print("\n=== 数据一致性验证 ===")
    #print("Query samples (前5个):", [f for f, _, _ in dataset.query][:5])
    #print("Gallery samples (前5个):", [f for f, _, _ in dataset.gallery][:5])
...
################################33
    ...
    #         if ('visual.transformer.resblocks.9' in n or 
    #             'visual.transformer.resblocks.10' in n or
    #             'visual.transformer.resblocks.11' in n or
    #             'visual.proj' in n or
    #             'visual.ln_post' in n)
    #     ]
    #     if visual_params:
    ...
    #     if hasattr(model.module, 'feature_head'):
    #         param_groups.append({
    #             'params': model.module.feature_head.parameters(),
    #             'lr': args.lr
    #         })
    ...
    #     #args.step_size = max(args.step_size // 2, 10)
    #     print(f"=> CLIP优化器: AdamW(lr={args.lr}), 可训练参数组: {len(param_groups)}")
    #     #####################################################
    ...
    #         param_groups = [
    #             {'params': backbone_params, 'lr': args.lr/2},
    ...
    #     optimizer = torch.optim.AdamW(
    #         [{'params': [p for n,p in model.named_parameters() if 'backbone' in n], 'lr': args.lr/2},
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use-dino-head', action='store_true')
